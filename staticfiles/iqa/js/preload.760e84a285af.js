(function() {
    ...
    function writeMetadata(cache, cacheName, manifest, saved, failed) {
        var now = Date.now();
        var metadata = {
            cache_name: cacheName,
            study_id: manifest.study_id,
            study_name: manifest.study_name,
            image_count: (manifest.urls || []).length,
            saved_count: saved,
            failed_count: failed,
            created_at: now,
    ...
    function cachedStatus(metadata, count) {
        ...
        return 'Cached ' + count + total
            + ' images. Expires ' + formatDate(metadata.expires_at) + '.';
    }
                    ...
                    .then(function(results) {
                        ...
                        if (!metadata) {
                            return caches.delete(cacheName).then(function() {
                                setStatus(
                                    card,
                                    'Saved images need refresh. Please preload again.'
                                );
                            });
                        }
