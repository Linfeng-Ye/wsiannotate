(function() {
    ...
    function setBusy(card, busy) {
        card.querySelectorAll('[data-preload-action]').forEach(function(btn) {
            if (btn.dataset.preloadAction !== 'preload') {
                btn.disabled = busy;
            }
        });
    }

    function setPreloadButton(card, text, disabled) {
        var button = card.querySelector('[data-preload-action="preload"]');
        if (!button) return;
        button.textContent = text;
        button.disabled = !!disabled;
    }

    function pausePreload(card) {
        var job = card._preloadJob;
        if (!job || !job.running) return;
        job.paused = true;
        setPreloadButton(card, 'Pausing...', true);
        setStatus(card, 'Pausing after current downloads finish...');
    }
        ...
        return registerWorker()
            ...
            .then(function(exists) {
                if (!exists) {
                    ...
                    setPreloadButton(card, 'Preload Images', false);
                    return null;
                }
                return caches.open(cacheName)
                    ...
                    .then(function(results) {
                        ...
                        if (!metadata && count) {
                            ...
                            setPreloadButton(card, 'Resume Preload', false);
                            return null;
                        }
    ...
    function preloadCard(card) {
        if (card._preloadJob && card._preloadJob.running) {
            pausePreload(card);
            return Promise.resolve();
        }
        ...
        card._preloadJob = job;

        setBusy(card, true);
        setPreloadButton(card, 'Pause Preload', false);
            ...
            .then(function(manifest) {
                ...
                return caches.open(cacheName)
                    ...
                    .then(function(result) {
                        ...
                        setStatus(
                            card,
                            cachedStatus(result.metadata, result.count)
                        );
                        setPreloadButton(
                            card,
                            job.failed ? 'Resume Preload' : 'Preload Images',
                            false
                        );
                        setBusy(card, false);
                        return null;
    ...
    function deleteCardCache(card) {
        card._preloadJob = null;
        setPreloadButton(card, 'Preload Images', true);
        setBusy(card, true);
